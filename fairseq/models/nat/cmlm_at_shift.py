...
class CMLMATModel(CMLMNATransformerModel):
    ...
    def forward(
        ...
        # change bos to eos for AT consistency
        prev_output_tokens[:, 0] = self.eos
        tgt_tokens[:, 0] = self.eos
        ...
        # decoding
        word_ins_out, word_ins_out_at, tgt_tokens_at, prev_output_tokens_at = self.decoder(
        ...
        word_ins_mask_at = word_ins_mask.new_ones(*word_ins_mask.size())
        word_ins_mask_at = tgt_tokens_at.ne(self.pad)
        word_ins_mask_at[:, :-1] = word_ins_mask[:, 1:] & word_ins_mask_at[:, :-1]
        ...
        return {
            ...
            "word_ins_at": {
                "out": word_ins_out_at, "tgt": tgt_tokens_at,
    ...
    def forward(
        ...
        **unused
        ...
        if not inference:
            ...
            # NAT tgt_tokens [eos, 1, 2, 3, 4] -> AT: [1, 2, 3, 4, eos]
            # NAT prev_output_tokens [eos, 1, m, 3, m] -> AT: [eos, 1, 2, 3, 4]
            prev_output_tokens_at = tgt_tokens.clone()
            tgt_tokens_at = tgt_tokens.clone()
            tgt_tokens_at[:, :-1] = prev_output_tokens_at[:, 1:]

            token_mask = tgt_tokens_at.ne(self.eos) & tgt_tokens_at.ne(self.unk) & tgt_tokens_at.ne(self.pad)
            eos_idxes = token_mask.sum(1, keepdim=True)
            bsz, seq_len = tgt_tokens_at.size()
            arange_mat = torch.arange(seq_len, 
                device=tgt_tokens_at.device).squeeze(0).repeat([bsz, 1])
            tgt_tokens_at = tgt_tokens_at.masked_fill(arange_mat==eos_idxes, self.eos)
            tgt_tokens_at = tgt_tokens_at.masked_fill(arange_mat>eos_idxes, self.pad)
            prev_output_tokens_at = prev_output_tokens_at.masked_fill(prev_output_tokens_at.eq(self.eos), self.pad)
            prev_output_tokens_at[:, 0] = self.eos
            decoder_out_at, _ = self.forward_at(
                prev_output_tokens_at,
                encoder_out=encoder_out,
                bottom_features=bottom_features,
                )
            if normalize:
                return F.log_softmax(decoder_out, -1), F.log_softmax(decoder_out_at, -1), tgt_tokens_at, prev_output_tokens_at
            else:
                return decoder_out, decoder_out_at, tgt_tokens_at, prev_output_tokens_at
...
@register_model_architecture("cmlm_at_shift", "cmlm_at_shift")
...
@register_model_architecture("cmlm_at_shift", "cmlm_at_shift_wmt_en_de")
