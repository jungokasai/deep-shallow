...
class TransformerDecoderLayer(nn.Module):
    ...
    *args.decoder_normalize_before* to ``True``.
    ...
    def __init__(
        ...
        self.skip_fc1 = getattr(args, "skip_fc1", False)
        self.skip_fc2 = getattr(args, "skip_fc2", False)
        if not self.skip_fc1:
            self.fc1 = self.build_fc1(self.embed_dim, args.decoder_ffn_embed_dim)
        if not self.skip_fc2:
            self.fc2 = self.build_fc2(args.decoder_ffn_embed_dim, self.embed_dim)
        if not (self.skip_fc1 and self.skip_fc2):
